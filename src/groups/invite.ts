...
module.exports = function (Groups){
	...
	Groups.invite = async function (groupName: string, uids: string | string[]) {
		...
		const notificationData = await Promise.all(uids.map((uid: string) => notifications.create({
			type: 'group-invite',
	...
	async function inviteOrRequestMembership(groupName:string, uids: string | string[], type:string): Promise<string[]> {
		uids = Array.isArray(uids) ? uids : [uids];
	...
	Groups.isInvited = async function (uids: string | string[], groupName: string) {
		return await checkInvitePending(uids, `group:${groupName}:invited`);
	};

	Groups.isPending = async function (uids: string | string[], groupName: string) {
